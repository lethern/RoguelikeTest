...
class GUILayout {
	...
	openWorkspace(layoutConfig) {
		const missingComponents = GoldenLayoutConfigUtils.extractConfigComponents(layoutConfig)
			.filter(c => !this.findComponent(c.componentType));

		if (missingComponents.length === 0) {
			GoldenLayoutConfigUtils.extractConfigComponents(layoutConfig)
				.forEach(c => this.focusComponent(c.componentType));
			return;
		}
	...
	#attachListeners() {
		...
		this.#layout.on('tabCreated', (tab) => {
			tab._dragListener.on('dragStart', () => { this.#preventLayoutSaving = true; });
		});
	...
	#flushResizeCommands() {
		for (const [id, data] of this.#resizeCache.entries()) {
			if (!this.#shouldSaveResize(id))
				continue;
			if (data.initialWidth !== data.currentWidth || data.initialHeight !== data.currentHeight) {
				editorActions.recordDispatched(new ResizeWidgetCommand({
					id,
