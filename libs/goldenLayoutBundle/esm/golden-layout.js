...
(function (ComponentItemConfig) {
    ...
    function fromResolved(resolvedConfig) {
        const result = {
            ...
            componentState: (0,deepExtendValue)(undefined, resolvedConfig.componentState),
        };
        return result;
    }
...
function parseSize(sizeString, allowableSizeUnits) {
    const { numericPart: digitsPart, firstNonNumericCharPart: firstNonDigitPart } = (0,splitStringAtFirstNonNumericChar)(sizeString);
...
(function (ResolvedComponentItemConfig) {
    ...
    function createCopy(original) {
        const result = {
            ...
            componentState: (0,deepExtendValue)(undefined, original.componentState),
        };
        return result;
    }
    ...
    function copyComponentType(componentType) {
        return (0,deepExtendValue)({}, componentType);
    }
...
class LayoutManager extends EventEmitter {
		...
		if (this._isInitialised === true) {
			if (this._groundItem === undefined) {
				...
				if (this._maximisedStack) {
					const { width, height } = (0,getElementWidthAndHeight)(this._containerElement);
					(0,setElementWidth)(this._maximisedStack.element, width);
					(0,setElementHeight)(this._maximisedStack.element, height);
	...
	createPopoutFromContentItem(item, window, parentId, indexInParent) {
		...
		if (parent === null) {
			...
			if (window === undefined) {
				...
				const { width, height } = (0,getElementWidthAndHeight)(item.element);
				window = {
	...
	removeDragSource(dragSource) {
		(0,removeFromArray)(dragSource, this._dragSources);
		dragSource.destroy();
	}
	/** @internal */
	...
	processMaximiseStack(stack) {
		...
		if (this._groundItem === undefined) {
			...
			const { width, height } = (0,getElementWidthAndHeight)(this._containerElement);
			(0,setElementWidth)(stack.element, width);
			(0,setElementHeight)(stack.element, height);
...
class ComponentContainer extends EventEmitter {
     ...
     */
    extendState(state) {
        const extendedState = (0,deepExtend)(this._state, state);
    ...
    enterDragMode(width, height) {
        this._width = width;
        this._height = height;
        (0,setElementWidth)(this._element, width);
        (0,setElementHeight)(this._element, height);
...
class BrowserPopout extends EventEmitter {
    ...
    popIn() {
        ...
        */
        ...
        const copiedGlInstanceLayoutConfig = (0,deepExtend)({}, glInstanceLayoutConfig);
     ...
     */
    createUrl() {
        const storageKey = 'gl-window-config-' + (0,getUniqueId)();
        const config = ResolvedLayoutConfig.minifyConfig(this._config);
        try {
            localStorage.setItem(storageKey, JSON.stringify(config));
        }
        catch (e) {
            throw new Error('Error while writing to localStorage ' + (0,getErrorMessage)(e));
        }
...
class DragProxy extends EventEmitter {
    ...
    createDragProxyElements(initialX, initialY) {
        ...
        this._element.style.left = (0,numberToPixels)(initialX);
        this._element.style.top = (0,numberToPixels)(initialY);
    ...
    setDropPosition(x, y) {
        ...
        this._element.style.left = (0,numberToPixels)(x);
        this._element.style.top = (0,numberToPixels)(y);
    ...
    setDimensions() {
        ...
        this._element.style.width = (0,numberToPixels)(width);
        this._element.style.height = (0,numberToPixels)(height);
        width -= (this._sided ? headerHeight : 0);
        height -= (!this._sided ? headerHeight : 0);
        this._proxyContainerElement.style.width = (0,numberToPixels)(width);
        this._proxyContainerElement.style.height = (0,numberToPixels)(height);
...
class DropTargetIndicator {
    ...
    highlightArea(area, margin) {
        this._element.style.left = (0,numberToPixels)(area.x1 + margin);
        this._element.style.top = (0,numberToPixels)(area.y1 + margin);
        this._element.style.width = (0,numberToPixels)(area.x2 - area.x1 - margin);
        this._element.style.height = (0,numberToPixels)(area.y2 - area.y1 - margin);
        this._element.style.display = 'block';
    }
    hide() {
        (0,setElementDisplayVisibility)(this._element, false);
    }
}
...
class Header extends EventEmitter {
    ...
    updateClosability() {
        ...
        if (this._closeButton !== null) {
            (0,setElementDisplayVisibility)(this._closeButton.element, isClosable);
        }
        if (this._popoutButton !== null) {
            (0,setElementDisplayVisibility)(this._popoutButton.element, isClosable);
        }
    ...
    updateTabSizes() {
        if (this._tabsContainer.tabCount > 0) {
            ...
            if (this._leftRightSided) {
                ...
                this._element.style.width = (0,numberToPixels)(headerHeight);
            }
            else {
                this._element.style.width = '';
                this._element.style.height = (0,numberToPixels)(headerHeight);
            }
            let availableWidth;
            if (this._leftRightSided) {
    ...
    processTabDropdownActiveChanged() {
        if (this._tabDropdownButton !== undefined) {
            (0,setElementDisplayVisibility)(this._tabDropdownButton.element, this._tabsContainer.dropdownActive);
        }
    }
    /** @internal */
...
class Splitter {
    constructor(_isVertical, _size, grabSize) {
        ...
        if (this._isVertical) {
            dragHandleElement.style.top = (0,numberToPixels)(-handleExcessPos);
            dragHandleElement.style.height = (0,numberToPixels)(this._size + handleExcessSize);
            this._element.classList.add("lm_vertical" /* Vertical */);
            this._element.style.height = (0,numberToPixels)(this._size);
        }
        else {
            dragHandleElement.style.left = (0,numberToPixels)(-handleExcessPos);
            dragHandleElement.style.width = (0,numberToPixels)(this._size + handleExcessSize);
            this._element.classList.add("lm_horizontal" /* Horizontal */);
            this._element.style.width = (0,numberToPixels)(this._size);
        }
    ...
    tryUpdateTabSizes(dropdownActive, availableWidth, activeComponentItem) {
        if (this._tabs.length > 0) {
            ...
            for (let i = 0; i < this._tabs.length; i++) {
                ...
                const tabMarginRight = (0,pixelsToNumber)(tabMarginRightPixels);
                ...
                if (activeIndex <= i) {
                    ...
                    const activeTabMarginRight = (0,pixelsToNumber)(activeTabMarginRightPixels);
                ...
                if (visibleTabWidth > availableWidth) {
                    ...
                    if (!tabOverlapAllowanceExceeded) {
                        ...
                        if (overlap < tabOverlapAllowance) {
                            for (let j = 0; j <= i; j++) {
                                const marginLeft = (j !== activeIndex && j !== 0) ? '-' + (0,numberToPixels)(overlap) : '';
                                this._tabs[j].element.style.zIndex = (0,numberToPixels)(i - j);
    ...
    bindComponent(container, itemConfig) {
        ...
        if (instantiator !== undefined) {
            ...
            if (itemConfig.componentState === undefined) {
                ...
                // make copy
                componentState = (0,deepExtendValue)({}, itemConfig.componentState);
            }
            let component;
            ...
            if (virtual) {
                if (component === undefined) {
                    ...
                    if (componentRootElement === undefined) {
                        ...
                        (0,ensureElementPositionAbsolute)(componentRootElement);
    ...
    handleContainerVirtualRectingRequiredEvent(container, width, height) {
        ...
        if (virtuableComponent === undefined) {
            ...
            if (rootElement === undefined) {
                ...
                rootElement.style.left = (0,numberToPixels)(left);
                const top = containerBoundingClientRect.top - this._goldenLayoutBoundingClientRect.top;
                rootElement.style.top = (0,numberToPixels)(top);
                (0,setElementWidth)(rootElement, width);
                (0,setElementHeight)(rootElement, height);
            }
        }
    }
    /** @internal */
    handleContainerVirtualVisibilityChangeRequiredEvent(container, visible) {
        ...
        if (virtuableComponent === undefined) {
            ...
            if (rootElement === undefined) {
                ...
                (0,setElementDisplayVisibility)(rootElement, visible);
            }
        }
    }
    /** @internal */
...
class ContentItem extends EventEmitter {
	...
	show() {
		...
		try {
			...
			(0,setElementDisplayVisibility)(this._element, true);
...
class ComponentItem extends ContentItem {
    ...
    /** @internal */
    enterDragMode(width, height) {
        (0,setElementWidth)(this.element, width);
        (0,setElementHeight)(this.element, height);
    ...
    updateNodeSize(force) {
        if (this.element.style.display !== 'none') {
            ...
            const { width, height } = (0,getElementWidthAndHeight)(this.element);
...
class GroundItem extends ComponentParentableItem {
    ...
    setSize(width, height) {
        if (width === undefined || height === undefined) {
            ...
            if (this.contentItems.length > 0) {
                (0,setElementWidth)(this.contentItems[0].element, width);
                (0,setElementHeight)(this.contentItems[0].element, height);
            }
...
class RowOrColumn extends ContentItem {
    ...
    setAbsoluteSizes() {
        ...
        for (let i = 0; i < this.contentItems.length; i++) {
            ...
            if (this._isColumn) {
                (0,setElementWidth)(this.contentItems[i].element, absoluteSizes.crossAxisSize);
                (0,setElementHeight)(this.contentItems[i].element, absoluteSizes.itemSizes[i]);
            }
            else {
                (0,setElementWidth)(this.contentItems[i].element, absoluteSizes.itemSizes[i]);
                (0,setElementHeight)(this.contentItems[i].element, absoluteSizes.crossAxisSize);
            }
        }
    }
    /**
    ...
    calculateAbsoluteSizes() {
        ...
        const { width: elementWidth, height: elementHeight } = (0,getElementWidthAndHeight)(this.element);
        let totalSize;
        let crossAxisSize;
        if (this._isColumn) {
    ...
    onSplitterDragStart(splitter) {
        ...
        const beforeWidth = (0,pixelsToNumber)(items.before.element.style[this._dimension]);
        const afterSize = (0,pixelsToNumber)(items.after.element.style[this._dimension]);
    ...
    onSplitterDrag(splitter, offsetX, offsetY) {
        ...
        const offsetPixels = (0,numberToPixels)(offset);
        if (this._isColumn) {
    ...
    onSplitterDragStop(splitter) {
        if (this._splitterPosition === null) {
            ...
            const items = this.getSplitItems(splitter);
            const sizeBefore = (0,pixelsToNumber)(items.before.element.style[this._dimension]);
            const sizeAfter = (0,pixelsToNumber)(items.after.element.style[this._dimension]);
            ...
            splitter.element.style.top = (0,numberToPixels)(0);
            splitter.element.style.left = (0,numberToPixels)(0);
...
(function (RowOrColumn) {
    ...
    function getElementDimensionSize(element, dimension) {
        if (dimension === 'width') {
            return (0,getElementWidth)(element);
        }
        else {
            return (0,getElementHeight)(element);
        }
    }
    ...
    function setElementDimensionSize(element, dimension, value) {
        if (dimension === 'width') {
            return (0,setElementWidth)(element, value);
        }
        else {
            return (0,setElementHeight)(element, value);
        }
    }
...
class Stack extends ComponentParentableItem {
    ...
    updateNodeSize() {
        if (this.element.style.display !== 'none') {
            const content = (0,getElementWidthAndHeight)(this.element);
            ...
            this._childElementContainer.style.width = (0,numberToPixels)(content.width);
            this._childElementContainer.style.height = (0,numberToPixels)(content.height);
            for (let i = 0; i < this.contentItems.length; i++) {
                this.contentItems[i].element.style.width = (0,numberToPixels)(content.width);
                this.contentItems[i].element.style.height = (0,numberToPixels)(content.height);
            }
            this.emit('resize');
            this.emitStateChangedEvent();
        }
    ...
    /** @internal */
    setupHeaderPosition() {
        (0,setElementDisplayVisibility)(this._header.element, this._header.show);
...
/************************************************************************/
export {  ApiError,  BindError,  BrowserPopout,  ComponentContainer,  ComponentItem,  ComponentItemConfig,  ConfigurationError,  ContentItem,  DragSource,  EventEmitter,  EventHub,  ExternalError, GoldenLayout as GoldenLayout,  Header,  HeaderedItemConfig,  I18nStrings,  ItemConfig,  ItemType,  JsonValue,  LayoutConfig, LayoutManager as LayoutManager,  LogicalZIndex,  LogicalZIndexToDefaultMap,  PopoutBlockedError,  PopoutLayoutConfig,  ResolvedComponentItemConfig,  ResolvedGroundItemConfig,  ResolvedHeaderedItemConfig,  ResolvedItemConfig,  ResolvedLayoutConfig,  ResolvedPopoutLayoutConfig,  ResolvedRootItemConfig,  ResolvedRowOrColumnItemConfig,  ResolvedStackItemConfig,  ResponsiveMode,  RootItemConfig,  RowOrColumn,  RowOrColumnItemConfig,  Side,  SizeUnitEnum,  Stack,  StackItemConfig,  StyleConstants,  Tab, VirtualLayout as VirtualLayout,  WidthOrHeightPropertyName,  formatSize,  formatUndefinableSize,  i18nStrings,  parseSize };
